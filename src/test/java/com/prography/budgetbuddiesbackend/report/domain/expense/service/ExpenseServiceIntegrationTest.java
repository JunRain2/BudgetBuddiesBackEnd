...
class ExpenseServiceIntegrationTest {
    ...
    void 금액이_0원일_때_등록_실패() {
        ...
        RegisterExpenseRequest req = new RegisterExpenseRequest(category.getId(), 0, "테스트", LocalDate.now().minusDays(1));
        // when & then
    ...
    void 금액이_1원일_때_등록_성공() {
        ...
        User user = userRepository.save(User.of());
        Category category = categoryRepository.findAll().get(0);
        RegisterExpenseRequest req = new RegisterExpenseRequest(category.getId(), 1, "테스트", LocalDate.now().minusDays(1));
        // when
        expenseService.registerExpense(req, user.getId());
        // then
        Expense expense = expenseRepository.findAll().stream().filter(e -> e.getUser().getId().equals(user.getId())).findFirst().orElseThrow();
    ...
    void 과거_날짜_등록_성공() {
        ...
        User user = userRepository.save(User.of());
        Category category = categoryRepository.findAll().get(0);
        RegisterExpenseRequest req = new RegisterExpenseRequest(category.getId(), 1000, "테스트", LocalDate.now().minusDays(1));
        // when
        expenseService.registerExpense(req, user.getId());
        // then
        Expense expense = expenseRepository.findAll().stream().filter(e -> e.getUser().getId().equals(user.getId())).findFirst().orElseThrow();
    ...
    void 미래_날짜_등록_실패() {
        ...
        User user = userRepository.save(User.of());
        Category category = categoryRepository.findAll().get(0);
        RegisterExpenseRequest req = new RegisterExpenseRequest(category.getId(), 1000, "테스트", LocalDate.now().plusDays(1));
        // when & then
        assertThatThrownBy(() -> expenseService.registerExpense(req, user.getId()))
                .isInstanceOf(NotRegisterExpenseException.class);
    }
