...
class ConsumptionGoalControllerIntegrationTest extends AbstractControllerTest {
	...
	void getUserConsumptionGoalsByMonth_withGoals() {
		...
		ConsumptionGoal goal = ConsumptionGoal.of(userId, category, 100000, yearMonthObj);
	...
	void batchUpdateConsumptionGoalCap_success() {
		...
		// 다른 카테고리 생성
		Category category2 = Category.of(userId, "테스트카테고리2");
		Category savedCategory2 = categoryRepository.save(category2);

		// 서로 다른 카테고리로 소비목표 생성
		ConsumptionGoal goal1 = ConsumptionGoal.of(userId, category, 100000, yearMonth);
		ConsumptionGoal goal2 = ConsumptionGoal.of(userId, savedCategory2, 200000, yearMonth);
		...
		List<GoalCapUpdate> updates = List.of(
			new GoalCapUpdate(savedGoal1.getId(), 150000),
			new GoalCapUpdate(savedGoal2.getId(), 250000)
		);
		BatchUpdateConsumptionGoalCapRequest request = new BatchUpdateConsumptionGoalCapRequest(updates);
	...
	void batchUpdateConsumptionGoalCap_negativeCap_failure() {
		...
		// 소비목표 생성
		ConsumptionGoal goal = ConsumptionGoal.of(userId, category, 100000, yearMonth);
		ConsumptionGoal savedGoal = consumptionGoalRepository.save(goal);

		List<GoalCapUpdate> updates = List.of(
			new GoalCapUpdate(savedGoal.getId(), -1000)
		);
		BatchUpdateConsumptionGoalCapRequest request = new BatchUpdateConsumptionGoalCapRequest(updates);
	...
	void batchUpdateConsumptionGoalCap_zeroCap_failure() {
		...
		List<GoalCapUpdate> updates = List.of(
			new GoalCapUpdate(savedGoal.getId(), 0)
		);
		BatchUpdateConsumptionGoalCapRequest request = new BatchUpdateConsumptionGoalCapRequest(updates);
	...
	void batchUpdateConsumptionGoalCap_oneWon_success() {
		...
		List<GoalCapUpdate> updates = List.of(
			new GoalCapUpdate(savedGoal.getId(), 1)
		);
		BatchUpdateConsumptionGoalCapRequest request = new BatchUpdateConsumptionGoalCapRequest(updates);
	...
	void batchUpdateConsumptionGoalCap_maxAmount_success() {
		...
		List<GoalCapUpdate> updates = List.of(
			new GoalCapUpdate(savedGoal.getId(), Integer.MAX_VALUE)
		);
		BatchUpdateConsumptionGoalCapRequest request = new BatchUpdateConsumptionGoalCapRequest(updates);
	...
	void getUserConsumptionGoalsByMonth_otherUserIsolation() {
		...
		// 다른 사용자의 소비목표 생성
		YearMonth yearMonth = YearMonth.now();
		ConsumptionGoal otherGoal = ConsumptionGoal.of(savedOtherUser.getId(), savedOtherCategory, 100000, yearMonth);
	...
	void batchUpdateConsumptionGoalCap_otherUserGoal_failure() {
		...
		List<GoalCapUpdate> updates = List.of(
			new GoalCapUpdate(savedOtherGoal.getId(), 200000)
		);
		BatchUpdateConsumptionGoalCapRequest request = new BatchUpdateConsumptionGoalCapRequest(updates);
	...
	void batchUpdateConsumptionGoalCap_multipleGoals_success() {
		...
		// 다른 카테고리들 생성
		Category category2 = Category.of(userId, "테스트카테고리2");
		Category category3 = Category.of(userId, "테스트카테고리3");
		Category savedCategory2 = categoryRepository.save(category2);
		...
		ConsumptionGoal goal1 = ConsumptionGoal.of(userId, category, 100000, yearMonth);
		ConsumptionGoal goal2 = ConsumptionGoal.of(userId, savedCategory2, 200000, yearMonth);
		ConsumptionGoal goal3 = ConsumptionGoal.of(userId, savedCategory3, 300000, yearMonth);
		ConsumptionGoal savedGoal1 = consumptionGoalRepository.save(goal1);
		ConsumptionGoal savedGoal2 = consumptionGoalRepository.save(goal2);
		...
		List<GoalCapUpdate> updates = List.of(
			new GoalCapUpdate(savedGoal1.getId(), 150000),
			new GoalCapUpdate(savedGoal2.getId(), 250000),
			new GoalCapUpdate(savedGoal3.getId(), 350000)
		);
		BatchUpdateConsumptionGoalCapRequest request = new BatchUpdateConsumptionGoalCapRequest(updates);
